...
public:
  ...
  bool isDirectory; // IFT320: add sub-directory functionality
...
public:
  ...
  Directory(int size, char *name, int _parentSector, int _sector);
  // IFT320: create named directory with hierarchy attributes
  ...
  // IFT320: Return sector data
  int GetParentSector();

  int GetSector();

  void MarkDirectory(char *name);
...
private:
  ...
  // IFT320: Add sectors referencing self and parent
  int parentSector;
  int selfSector;
};
